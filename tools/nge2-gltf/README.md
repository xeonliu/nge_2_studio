...
- node, mesh, primitive, vertex, triangle, bone, and texture counts;
...
HGMN animation, PSP sprites, Bezier/spline geometry, display-list jumps/calls,
morph targets, PBR material reconstruction, ISO input, and desktop integration
are not implemented. Duplicate typed keys are accepted only when the requested
HGOB/HGMS/HGPT candidates have identical data. PSP fixed-function render bits
are retained but not interpreted beyond texture selection and transparency.
...
the exported GLB back with `pygltflib`. They also cover both HGAR versions,
